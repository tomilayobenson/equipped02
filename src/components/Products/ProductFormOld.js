...
const productsReducer=(acc,action) => {
    switch (action.type) {
        case 'ADD_PRODUCT':
            return {
                ...acc,
                products: acc.products.concat([{...action.payload, id:uuid()}])
            };
            default:
                return acc
    }
}
export const ProductFormOld = () => {
    const [state,dispatch] = useReducer(productsReducer,initState);
    const {products} = state;
    const handleSubmit = (values, { resetForm }) => {
        console.log('form values:', values);
        // console.log('in JSON format:', JSON.stringify(values));
        dispatch({type:'ADD_PRODUCT', payload:values})
  ...
  return (
    <Container className="my-5">
        <h3 class="text-center display-4">Post an Item</h3>
        <Formik
            initialValues={{
                postingTitle:'',
